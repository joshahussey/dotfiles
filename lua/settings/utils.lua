-- Finds the root of the project by looking for a marker file in the current directory or any parent directory.
function Find_root(markers, directory, upward)
    if upward == nil then
        upward = true
    end
    if not upward == true and not upward == false then
        print("'Upward' must be true or false, defaulting to true.")
        upward = true
    end
    if directory == nil then
        directory = vim.fn.getcwd()
    end
	local dirname = vim.fs.dirname(vim.fs.find(markers, { path = directory, upward = upward })[1])
	return dirname
end
